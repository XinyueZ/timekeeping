...
public final class AppGuardService extends IntentService {
							...
							if( status == TextToSpeech.SUCCESS ) {
								...
								if( mTextToSpeech != null ) {
									TextToSpeechUtils.doSpeak(
											mTextToSpeech, !TextUtils.isEmpty( taskToSpeak ) ? String.format( "%s,%s", timeToSpeak, taskToSpeak ) :
														   timeToSpeak );

									AppGuardService.notify(
											getApplication(), !TextUtils.isEmpty( taskToSpeak ) ? String.format( "%s: %s", timeText, taskToSpeak ) :
															  timeText );
								} else {
									doneSpeak();
								}
	...
	private void prepareSpeak() {
		if( mWakeLock == null ) {
			...
			mWakeLock = pm.newWakeLock( largerThan17 ? PowerManager.PARTIAL_WAKE_LOCK : PowerManager.FULL_WAKE_LOCK, TAG );
		}
		mWakeLock.acquire();
